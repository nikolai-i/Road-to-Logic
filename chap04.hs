...
-- two sets are equal if they are a subset of each other.
...
-- if every element falls in the other set, then it's a subset.
...
unionSet, intersectSet, differenceSet :: Eq a => Set a -> Set a -> Set a
unionSet (Set xs) (Set ys) = Set (xs ++ ys)
intersectSet (Set xs) (Set []) = Set []
intersectSet (Set xs) (Set (y:ys)) | inList y xs 
